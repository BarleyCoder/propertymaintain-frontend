...
const AssignedTasks = () => {
    ...
    const filteredWorkOrders = workOrders.filter(w => {
        ...
        const propertyName = w.requestId?.propertyId?.name || 'Unknown';
        ...
        <div className="flex h-screen w-full overflow-hidden bg-[#f7f9ff]">
            ...
            <main className="flex-1 flex flex-col md:ml-64 overflow-hidden">
                ...
                <header className="flex justify-between items-center px-6 w-full sticky top-0 z-50 bg-white h-16 border-b border-[#c1c6d6]">
                    <div className="font-bold text-lg text-[#005bbf]">Assigned Tasks</div>
                ...
                <div className="flex-1 overflow-y-auto p-6">
                    <div className="max-w-[1280px] mx-auto space-y-6">
                        ...
                        {/* Page Header */}
                        <div>
                            <h1 className="text-3xl font-bold text-[#181c20]">Assigned Tasks</h1>
                            <p className="text-sm text-[#414754]">View and update your assigned maintenance work orders.</p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                            ...
                            <div className="bg-white border border-[#c1c6d6] rounded-xl p-6 flex items-center gap-4">
                                ...
                                <div>
                                    <p className="text-xs font-semibold text-[#414754] uppercase">Assigned</p>
                            ...
                            <div className="flex-1 min-w-[240px]">
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        placeholder="Search by ID, Category or Property"
                                        type="text"
                                        value={searchTerm}
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-left">
                                    ...
                                    <tbody className="divide-y divide-[#c1c6d6]">
                                        {loading ? (
                                            <tr>
                                                <td colSpan="6" className="px-6 py-8 text-center text-[#414754]">Loading tasks...</td>
                                            </tr>
                                        ) : filteredWorkOrders.length === 0 ? (
                                            <tr>
                                                <td colSpan="6" className="px-6 py-8 text-center text-[#414754]">
                                                    {workOrders.length === 0 ? 'No tasks assigned yet.' : 'No tasks match your filters.'}
                                                </td>
                                            </tr>
                                        ) : (
                                            filteredWorkOrders.map(order => (
                                                <tr key={order._id} className="hover:bg-[#f7f9ff] transition-colors">
                                                    <td className="px-6 py-4 text-sm font-bold text-[#005bbf]">
                                                        #{order._id ? order._id.substring(order._id.length - 6).toUpperCase() : 'N/A'}
                                                    </td>
                                                    <td className="px-6 py-4 text-sm">
                                                        {order.requestId?.propertyId?.name || 'Unknown Property'}
                                                    </td>
                                                    <td className="px-6 py-4 text-sm capitalize">
                                                        {order.requestId?.category || 'N/A'}
                                                    </td>
                                                    <td className="px-6 py-4 text-sm">
                                                        {order.scheduledDate ? new Date(order.scheduledDate).toLocaleDateString() : 'No deadline set'}
                                                    </td>
